...
application|language|notes|
:---------:|:------:|:----|
NFS Client|`C`|This is handled entirely by the netdata daemon. It can be configured via `netdata.conf`, section `[plugin:proc:/proc/net/rpc/nfs]`.
NFS Server|`C`|This is handled entirely by the netdata daemon. It can be configured via `netdata.conf`, section `[plugin:proc:/proc/net/rpc/nfsd]`.
