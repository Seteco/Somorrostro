...
It is even more tricky, because walking through the entire process tree takes some time itself. So, if you sum the utilization of all processes, you will have more CPU time that the reported total cpu time of the system. netdata solves this too, by adapting the per process cpu utilization to the total of the system. [netdata adds charts that document this normalization](https://london.my-netdata.io/default.html#menu_netdata_submenu_apps_plugin).
